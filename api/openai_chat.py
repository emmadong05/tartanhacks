...
class OpenAiManager:
    ...
    def num_tokens_from_messages(self, messages, model='gpt-4o'):
        ...
        try:
            if self.tiktoken_encoder == None:
                # We store this value so we don't have to check again every time
                    ...
                    if key == 'role':
                        ...
                        for message_data in value:
                            for content_key, content_value in message_data.items():
                                if content_key == 'type':
                                    num_tokens += len(
                                        self.tiktoken_encoder.encode(content_value))
    ...
    def chat_with_history(self, prompt="", image_path="", local_image=True):
            ...
            if image_path != "":
                ...
                if local_image:
                    try:
                        ...
                        print(
                            "[red]ERROR: COULD NOT BASE64 ENCODE THE IMAGE. PANIC!!")
                        return None
                else:
                    url = image_path  # The provided image path is a URL
                new_image_content = {
        ...
        if self.logging:
            print(f"[coral]Chat History has a current token length of {
                  self.num_tokens_from_messages(self.chat_history)}")
        while self.num_tokens_from_messages(self.chat_history) > 128000:
            # We skip the 1st message since it's the system message
            self.chat_history.pop(1)
            if self.logging:
                print(f"Popped a message! New token length is: {
                      self.num_tokens_from_messages(self.chat_history)}")
